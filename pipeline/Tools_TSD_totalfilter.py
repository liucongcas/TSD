...
def filterRT_Fus(fr,list_filter):
    ...
    for y in range(len(list_filter_final)):
        ...
        #print "lala2",dict_total[par2]
        del dict_total[par2]
    for k in dict_total.keys():
        s1=dict_total[k]
        s2=[s1[0],s1[1],s1[0],s1[2],s1[3],s1[4]]
        list_filter.append(s2)
    return list_filter
...
def filter_neigborGene_adjacent(list_filter_fus,split_gene):
    ...
    for x1 in range(len(list_filter_fus)):
        ...
        strand_l=list_filter_fus[x1][3]
        strand_r=list_filter_fus[x1][4]
        left_gene=False
        right_gene=False
        between_gene=[]
        for x2  in range(len(single_list)):
            ...
            if strand_l==strand_r:  #when the strand of two sites are same, find the interval gene in the same strand
                ...
                if TSE_r >= gene_l and TSE_r <= gene_r:
                    right_gene = True
        #print between_gene,TSE_l,TSE_r
...
def filterReadthrough():
    ...
    for i in range(len(fr)):
        xr = fr[i]
        xr2=[[xr[0], xr[1], xr[3], xr[4], xr[5]]]
        if xr[0]==xr[2]:
            if xr[4] == xr[5] == "+" or xr[4] == xr[5] == "1":
                if float(xr[1]) < float(xr[3]):
                    if abs(float(xr[3]) - float(xr[1])) > 1000000:
                        list_filter.append(xr)
                else:
                    list_filter = filterRT_Fus(xr2, list_filter)
            elif xr[4] == xr[5] == "-" or xr[4] == xr[5] == "-1":
                if float(xr[1]) > float(xr[3]):
                    if abs(float(xr[1]) - float(xr[3])) > 1000000:
                        list_filter.append(xr)
                else:
                    list_filter = filterRT_Fus(xr2, list_filter)
            else:
